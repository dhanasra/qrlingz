    ...
    return BlocListener<ImageBloc, ImageState>(
      listener: (context, state) {
        if(state is ImageFetched){
          vm.tempData.value =
                          vm.tempData.value.copyWith(logo: state.url);
        }
      },
      child: ListView(
                ...
                return Row(
                  ...
                  children: [
                    StyledButton(
                        w: 140,
                        onClick: () =>
                            context.read<ImageBloc>().add(PickAndUploadImage()),
                        text: "Pick Image"),
                    if (value.logo != null) 16.w(),
                    if (value.logo != null)
                      StyledButton(
                          w: 140,
                          outlined: true,
                          onClick: () {
                            vm.tempData.value =
                                vm.tempData.value.copyWith(logo: null);
                          },
                          text: "Remove")
                  ],
                );
          ...
          GridView.builder(
              ...
              itemBuilder: (_, idx) {
                return InkWell(
                    onTap: () {
                      vm.tempData.value =
                          vm.tempData.value.copyWith(logo: items[idx]);
                    },
